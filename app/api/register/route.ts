...
export async function POST(request: Request) {
  ...
  const lead = {
    ...
    projectTag: projectTag?.trim() || PROJECT.tag,
    source: source?.trim() || PROJECT.source,
  ...
  const webhookUrl =
    process.env.REGISTRATION_WEBHOOK_URL ?? process.env.NEXT_PUBLIC_WEBHOOK_URL;
  if (webhookUrl) {
    try {
